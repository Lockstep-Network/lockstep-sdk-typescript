 ...
 */
 ...
 export class LockstepApi {
    ...
    */
   public async request<T>(method: string, path: string, options: unknown, body: unknown): Promise<LockstepResponse<T>> {
    
    const result = await fetch(new url.URL(path, this.serverUrl).href, {
      method: method,
    ...
    if (response.success) {
      response.value = result.body as unknown as T;
    } else {
      response.error = result.body as unknown as T;
    }
