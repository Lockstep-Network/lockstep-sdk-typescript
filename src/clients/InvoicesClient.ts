...
import { InvoiceSummaryModelInvoiceSummaryTotalsModelSummaryFetchResult } from "..";
...
export class InvoicesClient {
   ...
   */
  deleteInvoice(id: string): Promise<LockstepResponse<DeleteResult>> {
    const url = `/api/v1/Invoices/${id}`;
   ...
   */
  retrieveInvoiceWorkflowStatusHistories(id: string): Promise<LockstepResponse<InvoiceWorkflowStatusHistoryModel[]>> {
    const url = `/api/v1/Invoices/${id}/workflow-status-history`;
    return this.client.request<InvoiceWorkflowStatusHistoryModel[]>("get", url, null, null);
  }
   ...
   */
  createInvoices(filename: string, body: InvoiceModel[], customerCompanyId?: string): Promise<LockstepResponse<InvoiceModel[]>> {
   ...
   */
  deleteInvoices(body: BulkDeleteRequestModel): Promise<LockstepResponse<DeleteResult>> {
    const url = `/api/v1/Invoices`;
   ...
   * @param id The unique Lockstep Platform ID number of this invoice; NOT the customer's ERP key
   */
  checkinvoicePDF(id: string): Promise<LockstepResponse<Blob>> {
    const url = `/api/v1/Invoices/${id}/pdf`;
   ...
   * @param pageNumber The page number for results (default 0). See [Searchlight Query Language](https://developer.lockstep.io/docs/querying-with-searchlight)
   */
  queryInvoiceSummaryView(filter?: string, include?: string, order?: string, pageSize?: number, pageNumber?: number): Promise<LockstepResponse<InvoiceSummaryModelInvoiceSummaryTotalsModelSummaryFetchResult>> {
    ...
    return this.client.request<InvoiceSummaryModelInvoiceSummaryTotalsModelSummaryFetchResult>("get", url, options, null);
  }
